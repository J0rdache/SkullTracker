# Use various hand gestures to print chars or strs in terminal / window
import cv2, time

hand_classifier = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_hand.xml")

video_capture = cv2.VideoCapture(0)


def detect_nearest_hand(vid):
    gray_image = cv2.cvtColor(vid, cv2.COLOR_BGR2GRAY)
    hands = hand_classifier.detectMultiScale(gray_image, 1.1, 5, minSize=(40, 40))

    if len(hands) == 0:
        return None

    return max(hands, key=lambda rect: rect[2] * rect[3])
